...
from schemas.blog import BlogCreate, ShowBlog, BlogUpdate
from db.repository.blog import create_new_blog, retrieve_blog, retrieve_all_blogs, \
...
def get_all_blogs(db: Session=Depends(get_db)):
    ...
    if not blogs:
        raise HTTPException(detail="There is no blog!", status_code=status.HTTP_404_NOT_FOUND)
    return blogs
